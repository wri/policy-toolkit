...
def apply(model, criterion, batch, targets):
    ...
    loss = criterion(pred.reshape((1, 2)), torch.autograd.Variable(targets))
    return pred, loss
...
class Model(nn.Module):
    ...
    def forward(self, batch):
        ...
        outputs, (ht, ct) = self.gru(batch, self.hidden)
...
model = train(model=model,
              ...
              train_generator=training_generator,
              val_generator=validation_generator)
torch.save(model, 'forth_model.pth')
# model = torch.load('/home/yg2619/capstone/third_model.pth')
test(model, loss_fn, test_generator)
